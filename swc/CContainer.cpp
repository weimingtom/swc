...
BOOL CContainer::OnEraseBkgnd(HDC hDC) 
{
	...
	if (m_Numtabs == 0)
	{
		...
		pDC.FillRect((LPCRECT)m_rect,&cbr);
		cbr.DeleteObject();
	}
	return TRUE;
}
...
BOOL CContainer::OnPaint(HDC hDC) 
{
	...
	if (m_alnTab!=ALN_TOP)
	{
		...
		MemDC.FillRect((LPCRECT)m_rectTab,&cbr);
	...
	else
	{
		...
		for (int i=0; i< 2; i++)
		{
			MemDC.Draw3dRect((LPCRECT)rc,CDrawLayer::GetRGBColorFace(),CDrawLayer::GetRGBColorFace());
			rc.DeflateRect(1,1);
		}
		
		rc.InflateRect(3,3);
		if (m_bDrawFrame)
			MemDC.Draw3dRect((LPCRECT)rc,GetSysColor(COLOR_BTNSHADOW),GetSysColor(COLOR_BTNSHADOW));
		else
		{
			CRect mrc=m_rectDraw;
			mrc.top+=nVal;
			MemDC.Draw3dRect((LPCRECT)mrc,GetSysColor(COLOR_BTNSHADOW),GetSysColor(COLOR_BTNSHADOW));

		}
		rc=m_rectDraw;
		rc.bottom=rc.top+3;
		MemDC.FillRect((LPCRECT)rc,&cbr);
...
void CContainer::OnNcPaint(HRGN hRgn)
{
	...
	m_dc.FillRect((LPCRECT)rcWindow, &cb);
	if (m_bCaption)
		DrawGripper(&m_dc,rcWindow);
	dc.IntersectClipRect((LPCRECT)rcWindow);
    dc.ExcludeClipRect((LPCRECT)rcClient);//asi evitamos el parpadeo
...
void CContainer::DrawGripper(CGDI* dc,CRect rcWin)
{
	...
	dc->FillRect((LPCRECT)rc, &cb);
	cb.DeleteObject ();
	dc->Draw3dRect((LPCRECT)rc,CDrawLayer::GetRGBMenu(),CDrawLayer::GetRGBMenu());
...
void CContainer::Drawtabs(CGDI* dc)
{
	...
	for (int iCont=0; iCont< m_Numtabs;iCont++)
	{
		...
		if (theApp->m_WinMain->m_sizeImagDocks.cx+m_sChar.cx < ct->rect.Width()-6)  
		{
			...
			CToolTip.SetToolRect(GetSafeHwnd(), (LPCRECT)CRect(0,0,0,0),iCont+1);
		}
		else
		{
...
CWin*  CContainer::GetNumWnd(int m_numtab) 
{
		...
		else
		{
			...
			if (HIWORD(pw)== 0/*NULL*/) return NULL;
			else
...
void CContainer::Addtab(CWin* pParent, LPCSTR lpMsg, UINT uHmenu)
{
	...
	CToolTip.AddTool(GetSafeHwnd(),ctn->lpMsg,(LPCRECT)tempRect,m_Numtabs);
	RecalLayout();
}
		...
		else
		{
			...
			for (iCont=0; iCont< m_Numtabs;iCont++)
			{
				...
				{	
					...
					SetCurtab(iCont);
					CToolTip.UpdateTipText(GetSafeHwnd(), ct->lpMsg,(LPCRECT)CRect(0,0,0,0), m_TabMove+1);
					CToolTip.UpdateTipText(GetSafeHwnd(), Temp->lpMsg,(LPCRECT)CRect(0,0,0,0), iCont+1);
