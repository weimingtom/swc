...
void CWorkTab::PintaSeparador(CGDI* dc)
{
	...
	if (!bIniciaArrastre)
	{
		dc->Draw3dRect((LPCRECT)m_rClient, ::GetSysColor(BLACK_PEN),
			::GetSysColor(COLOR_BTNHIGHLIGHT));
		m_rClient.DeflateRect(1, 1);
		dc->Draw3dRect((LPCRECT)m_rClient, ::GetSysColor(COLOR_BTNHIGHLIGHT),
...
BOOL CWorkTab::OnPaint(HDC hDC)
{
	...
	rc.top += 1;
	rc.bottom = rc.top + 1;
	dc.Draw3dRect((LPCRECT)rc, CDrawLayer::GetRGBColorFace(), CDrawLayer::GetRGBColorFace());
...
void CWorkTab::Drawtabs(CGDI* dc)
{
	...
	HRGN hRgn= CreateRectRgnIndirect((LPCRECT)rectRecorte);
...
BOOL CWorkTab::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_btndesplaza.PtInRect(point) && !bIniciaArrastre)
	{
		...
		SetCapture();
		InvalidateRect((LPCRECT)m_btndesplaza, TRUE);
	}
	else
	{
