	...
	if (m_rcSpplitter.PtInRect(point) && !bIniciaArrastre)
		{
			...
			SetCapture();
			InvalidateRect((LPCRECT)m_rcSpplitter, TRUE);
		}
}
...
void CSplitter::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    if (bIniciaArrastre==TRUE)
    {
		...
		InvalidateRect((LPCRECT)m_rcSpplitterOld);
		RecalLayout();
		dc.DeleteDC();
	}
}
...
void  CSplitter::RecalLayout()
{
		...
		if (pWndLeft== NULL || pWndRight==NULL)
		{
			...
			else if (pWndLeft==NULL)
			{
				if (pWndLeft->IsWindowVisible())
					pWndLeft->MoveWindow((LPCRECT)m_rectCli);
			}
			else
			{
				if (pWndRight->IsWindowVisible())
					pWndRight->MoveWindow((LPCRECT)m_rectCli);
			}
		}
	...
	pWndLeft->MoveWindow((LPCRECT)m_rctemp);
	pWndLeft->Invalidate();
	...
	m_rctemp.DeflateRect(1,1);
	pWndRight->MoveWindow((LPCRECT)m_rctemp);
...
void CSplitter::DrawSplitter(CRect rcSeparator,CRect rcLeft,CRect rcRight,CGDI* pDC)
{
	...
	{
		pDC->Draw3dRect((LPCRECT)rcLeft,mclrLeftTop,mclrBottomRight);
		pDC->Draw3dRect((LPCRECT)rcRight,mclrLeftTop,mclrBottomRight);
	}
	else
	{
		pDC->Draw3dRect((LPCRECT)rcLeft,CDrawLayer::GetRGBColorShadow(),CDrawLayer::GetRGBColorShadow());
		pDC->Draw3dRect((LPCRECT)rcRight,CDrawLayer::GetRGBColorShadow(),CDrawLayer::GetRGBColorShadow());
	}
